    ...
    let ob = new window.MutationObserver(mutations => {
        mutations.forEach(mutation => {
            mutation.addedNodes.forEach(node => {
                ...
                let addr_el = document.querySelector('div.address');
                if (!addr_el) {
                    return;
                }
                ...
                GM_xmlhttpRequest({
                    ...
                    onload: res => {
                        ...
                        addr_el.insertAdjacentElement('afterend', map_el);

                        let map = L.map(map_el).setView([r0.lat, r0.lon], 17);
                        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(map);
                        // L.marker([r0.lat, r0.lon]).addTo(map).bindPopup('House').openPopup();
                    },
                    url: url,
                });
            });
